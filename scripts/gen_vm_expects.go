...
func parseFlags() {
	...
	if len(args) > 0 {
		...
		f, err := os.Open(name)
		if err != nil {
			log.Fatalln("failed to open %v: %v", name, err)
		}
		args = args[1:]
		in = f
	}
	...
	if len(args) > 0 {
		...
		if err != nil {
			log.Fatalln("failed to create %v: %v", name, err)
		}
		args = args[1:]
		out = f
	}
...
var expectMethod = regexp.MustCompile(`func \(vmt vmTestCase\) expect(.+?)\((.+?)\) vmTestCase`)
...
func run(ctx context.Context) error {
	...
	for sc.Scan() {
		if match := expectMethod.FindSubmatch(sc.Bytes()); len(match) > 0 {
			buf.WriteString("func expectVM")
			buf.Write(match[1])
			buf.WriteString("(")
			buf.Write(match[2])
			buf.WriteString(") func(vmTestCase) vmTestCase {\n")
			buf.WriteString("  return func(vmt vmTestCase) vmTestCase {\n")
			buf.WriteString("    return vmt.expect")
			buf.Write(match[1])
			buf.WriteString("(")

			for i, part := range bytes.Split(match[2], []byte(",")) {
				if i > 0 {
					buf.WriteString(", ")
				}
